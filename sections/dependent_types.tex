...
There is a difference between \emph{parameters}, used before the colon, and \emph{indices} of a datatype.The datatype $\mathbb{V}$ is parameterised by a type A and indexed over $\mathbb{N}$\cite{norell:deptyped}.
...
\begin{codesnippet}[mathescape=true, caption={Definition of \emph{head} function in Agda}, label={codeSnippet:vector_head}]
head$\mathbb{V}$ : $\forall$ {$\ell$} {A : Set $\ell$} {n : $\mathbb{N}$} $\rightarrow$ 
        $\mathbb{V}$ A suc(n) $\rightarrow$ A
head$\mathbb{V}$ (x :: _) = x
\end{codesnippet}

One step further the function \emph{nth$\mathbb{V}$} also has an additional implicit parameter m of type $\mathbb{N}$ which represents the length of the input vector. 
The second explicit parameter is a proof that n is less than m.
In contrast to the function \emph{nth} this function returns always an A instead of a maybe A as it was before.
